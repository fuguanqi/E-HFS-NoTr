function [C,stage_EP,stage_ETI]=timingByDP(job_seq,assign,A,j,prob)
...
for m=1:prob.n_M(j)
    mn=numel(mac_seqs{m});
    if mn>=1
        [mC,m_EP,m_ETI]=cal_1m(mac_seqs{m},j,m,A,prob);
...
function [best_C,best_EP,best_ETI]=cal_1m(job_seq,j,m,A,prob)
A=A(job_seq);
...
for v=1:3
    ...
    if m_ETI+m_EP<best_ETI+best_EP
        best_ETI=m_ETI;
        best_C=C;
        best_EP=m_EP;
    end
